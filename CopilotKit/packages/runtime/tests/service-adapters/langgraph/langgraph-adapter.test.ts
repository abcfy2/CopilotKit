...
import { ActionExecutionMessage, ResultMessage, TextMessage } from "../../../src/graphql/types/converted";
...
function createSimpleAgent(responses: string[]) {
  const fakeLLM = new FakeStreamingChatModel({
    responses: responses.map((response) => new AIMessage({ content: response })),
  });
...
function createThinkingAgent(thinkingSteps: string[]) {
  const chunks = thinkingSteps.map(step => new AIMessageChunk({ content: step }));
...
function createAgentWithToolCalls(toolCalls: any[]) {
  const chunks = toolCalls.map(toolCall => new AIMessageChunk({
    content: "",
    tool_calls: [toolCall]
  }));
...
function createToolAgent(tools: any[]) {
  const fakeLLM = new FakeStreamingChatModel({
    responses: [new AIMessage({ content: "I'll use the available tools to help you." })]
  });
...
function createCustomEventAgent(customEvents: Array<{ name: string, data: any; }>) {
  const fakeLLM = new FakeStreamingChatModel({
...
function createMetadataFilteringAgent(shouldEmitMessages: boolean, shouldEmitToolCalls: boolean) {
  const chunks = [
    new AIMessageChunk({
      content: shouldEmitMessages ? "This message respects metadata" : "This should be filtered"
    })
  ];
...
describe("LangGraphServiceAdapter", () => {
  ...
  describe("Thinking Chain Visualization", () => {
    ...
    it("should handle complex thinking with array content", async () => {
      ...
      const chunks = [
        new AIMessageChunk({
          content: [{ type: 'text', text: 'Analyzing the problem systematically...' }]
        })
      ];
  ...
  describe("Advanced Error Handling and Edge Cases", () => {
    ...
    it("should handle high-frequency streaming events", async () => {
      ...
      const manyChunks = Array.from({ length: 100 }, (_, i) =>
        new AIMessageChunk({ content: `Token ${i} ` })
      );
  ...
  describe("CopilotKit Specific Features", () => {
    ...
    it("should handle complex message and action conversion", async () => {
      ...
      actionMessage.arguments = { param1: "value1", param2: { nested: "value" } };
